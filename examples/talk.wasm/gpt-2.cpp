...
std::string gpt2_gen_text(gpt2_context *ctx, const char *text, int max_tokens, std::string apiKey)
{
    std::string data = R"(
    {
    ...
    auto onsuccess = [&](emscripten_fetch_t *fetch) {
        result += std::string((char*)fetch->data, fetch->numBytes);
    ...
    {
        std::string str = "Bearer " + apiKey;
        c_format = str.c_str();
    }
    const char* headers[] = { "Content-Type", "application/json", "Authorization", c_format, 0};
    attr.requestHeaders = headers;
    attr.requestData = data.c_str();
    attr.requestDataSize = data.size();
    emscripten_fetch(&attr, "https://api.openai.com/v1/completions");
